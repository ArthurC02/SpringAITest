...
public sealed class WorkflowDesignerCompilerTests
{
    ...
    // 「沒有回應」不只有 HttpRequestException/timeout:socket 層的 IOException 同樣不是編譯結果。
    // 三者都必須 502 —— 曾經只 catch 前兩種,IOException 會逃逸成 500(對 Workflow 看起來像可重試)。
    [Fact] public async Task Validate_TransportAndTimeout_FailClosed() { foreach (var error in new Exception[] { new HttpRequestException("down"), new TaskCanceledException("timeout"), new IOException("socket reset") }) { var compiler = Build(new Handler(_ => Task.FromException<HttpResponseMessage>(error))); var ex = await Assert.ThrowsAsync<ApiException>(() => compiler.ValidateAsync("orchestrator", "{}", "{}", "demo-a", default)); Assert.Equal(502, ex.Status); } }
