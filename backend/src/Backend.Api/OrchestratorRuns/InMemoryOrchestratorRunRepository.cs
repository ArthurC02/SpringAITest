    ...
    IAgentRunRepository? agentRuns = null) : IOrchestratorRunRepository
{
    ...
    {
        if (request.RunKind is not ("worker" or "verifier") || request.Attempt < 1 || request.WriteIntent
            || request.TaskEnvelope is not { ValueKind: JsonValueKind.Object } || string.IsNullOrWhiteSpace(request.TaskId)) return null;
        ValidatedOrchestratorTaskEnvelope envelope;
        try { envelope = OrchestratorTaskEnvelope.Validate(request.TaskEnvelope); }
        ...
        lock(_gate)
        {
            ...
            using var doc=JsonDocument.Parse(root.Snapshot);var pin=Pin(doc.RootElement,request.RunKind,request.AgentId,request.AgentRevision);
            ...
            if(root.Children.Count>=limits.GetProperty("max_child_runs").GetInt32()||root.Children.Count(x=>x.Status is "queued" or "running")>=limits.GetProperty("max_concurrency").GetInt32()||root.Children.Any(x=>x.Task==request.TaskId&&x.Attempt==request.Attempt))return null;
            ...
            if(agentRuns is not null)
            {
                ...
                var started=childRuns.CreateOrchestratorChildAsync(tenant,user,root.Role,Strings(caller.GetProperty("groups")),caps,source.Value.Agent,source.Value.Workflow,new OrchestratorChildSnapshotProvenance(rootRunId,request.TaskId.Trim(),request.Attempt),request.RunKind,pin.Value.TokenCap,canonicalEnvelope,$"orchestrator-child:{rootRunId:D}:{request.TaskId}:{request.Attempt}:{request.RunKind}",ct).GetAwaiter().GetResult();
            ...
            var artifact=JsonSerializer.SerializeToElement(new{orchestrator_root_run_id=rootRunId,task_id=request.TaskId.Trim(),attempt=request.Attempt,run_kind=request.RunKind,root_snapshot_hash=root.Hash,agent_snapshot_hash=pin.Value.Hash,agent_run_id=agentRunId,command_id=commandId,command_input=commandInput});
            var child=new Child(Guid.NewGuid(),request.TaskId.Trim(),request.Attempt,request.RunKind,request.AgentId,request.AgentRevision,pin.Value.Workflow,pin.Value.Revision,pin.Value.Hash,agentRunId,commandId,canonicalEnvelope,artifact);root.Children.Add(child);root.Events.Add(new(root.Events.Count+1,"child_created",root.Hash,JsonDocument.Parse("{}").RootElement.Clone(),DateTime.UtcNow));return new(child.Id,rootRunId,child.Task,child.Attempt,child.Kind,child.AgentId,child.AgentRevision,child.WorkflowId,child.WorkflowRevision,child.Hash,child.Status,child.AgentRunId,child.CommandId);
        }
    }
    ...
    { lock(_gate){if(!_runs.TryGetValue(rootRunId,out var root)||root.Tenant!=tenant||root.User!=user)return Task.FromResult<OrchestratorChildStatusResponse?>(null);var child=root.Children.SingleOrDefault(x=>x.Id==childId);if(child is null)return Task.FromResult<OrchestratorChildStatusResponse?>(null);AgentRunResponse? run=agentRuns?.GetAsync(tenant,user,child.AgentRunId,ct).GetAwaiter().GetResult();if(run is not null){var prior=child.Status;child.Status=run.Status;if(prior!=child.Status&&child.Status is "completed" or "failed" or "cancelled")root.Events.Add(new(root.Events.Count+1,"child_terminal",root.Hash,JsonDocument.Parse(JsonSerializer.Serialize(new{child_id=child.Id,agent_run_id=child.AgentRunId,status=child.Status})).RootElement.Clone(),DateTime.UtcNow));}var output=run?.Result??JsonDocument.Parse("{}").RootElement.Clone();var citations=output.ValueKind==JsonValueKind.Object&&output.TryGetProperty("citations",out var cits)&&cits.ValueKind==JsonValueKind.Array?cits.Clone():JsonDocument.Parse("[]").RootElement.Clone();return Task.FromResult<OrchestratorChildStatusResponse?>(new(child.Id,rootRunId,child.Task,child.Attempt,child.Kind,child.AgentId,child.AgentRevision,child.WorkflowId,child.WorkflowRevision,child.Hash,child.AgentRunId,child.Status,output,citations,run?.ErrorCode,run?.ErrorMessage));}}
    ...
    { lock(_gate){if(!_runs.TryGetValue(rootRunId,out var root)||root.Tenant!=tenant||root.User!=user||request.ContextRound<1||request.CurrentContext is not {ValueKind:JsonValueKind.Object})return Task.FromResult<OrchestratorContextAcquireResponse?>(null);using var snapshot=JsonDocument.Parse(root.Snapshot);var authority=snapshot.RootElement.GetProperty("authority");var tools=authority.GetProperty("context_tools").EnumerateArray().Select(x=>x.GetString()!).OrderBy(x=>x,StringComparer.Ordinal);var knowledge=authority.GetProperty("knowledge_sources").EnumerateArray().Select(x=>x.GetString()!).OrderBy(x=>x,StringComparer.Ordinal);if(!tools.SequenceEqual((request.AllowedTools??[]).OrderBy(x=>x,StringComparer.Ordinal))||!knowledge.SequenceEqual((request.AllowedKnowledgeSources??[]).OrderBy(x=>x,StringComparer.Ordinal)))return Task.FromResult<OrchestratorContextAcquireResponse?>(null);var missing=tools.Select(x=>"context-tool:"+x).Concat(knowledge.Select(x=>"knowledge-source:"+x)).DefaultIfEmpty("context-adapter-unavailable").ToArray();return Task.FromResult<OrchestratorContextAcquireResponse?>(new(false,JsonDocument.Parse("{}").RootElement.Clone(),[],missing));}}
    public Task<OrchestratorRunWriteResult> CancelAsync(string tenant,string user,Guid id,string? reason,string key,CancellationToken ct) { lock(_gate) { if(!_runs.TryGetValue(id,out var x)||x.Tenant!=tenant||x.User!=user)return Task.FromResult(new OrchestratorRunWriteResult(OrchestratorRunWriteStatus.NotFound));if(_cancelKeys.TryGetValue((tenant,user,id,key),out var replay))return Task.FromResult(new OrchestratorRunWriteResult(OrchestratorRunWriteStatus.Replay,replay,Replayed:true)); if(x.Status is "completed" or "failed" or "cancelled") return Task.FromResult(new OrchestratorRunWriteResult(OrchestratorRunWriteStatus.InvalidState,Message:"Run is terminal")); x.Status="cancelled";foreach(var child in x.Children.Where(child=>child.Status is "queued" or "running")){if(agentRuns is not null)agentRuns.CancelAsync(tenant,user,child.AgentRunId,reason,$"orchestrator-cancel:{id:D}:{child.AgentRunId:D}",ct).GetAwaiter().GetResult();child.Status="cancelled";}x.CancelRequested=true;x.Version++;x.Updated=DateTime.UtcNow;x.Events.Add(new(x.Events.Count+1,"run_cancelled",x.Hash,JsonDocument.Parse(JsonSerializer.Serialize(new{reason})).RootElement.Clone(),x.Updated));var response=ToResponse(x);_cancelKeys[(tenant,user,id,key)]=response;return Task.FromResult(new OrchestratorRunWriteResult(OrchestratorRunWriteStatus.Success,response)); } }
    private static OrchestratorRunResponse ToResponse(Entry x) { using var d=JsonDocument.Parse(x.Snapshot);return new(x.Id,x.OrchestratorId,x.OrchestratorRevision,x.Conversation,x.WorkflowId,x.WorkflowRevision,x.Hash,x.Status,x.CancelRequested,x.Version,x.Deadline,Budgets(d.RootElement),x.Created,x.Updated); }
    ...
    private sealed class Entry(Guid id,Guid commandId,string tenant,string user,string role,Guid oid,int orev,string conversation,Guid wid,int wrev,string snapshot,string hash,DateTime deadline,DateTime now) { public Guid Id=id,CommandId=commandId;public string Tenant=tenant,User=user,Role=role,Conversation=conversation,Snapshot=snapshot,Hash=hash,Status="queued";public Guid OrchestratorId=oid,WorkflowId=wid;public int OrchestratorRevision=orev,WorkflowRevision=wrev;public long Version=1,LeaseGeneration;public bool CancelRequested,CommandCompleted,DispatchCompleted;public string? ClaimTokenHash;public DateTime ClaimExpiresAt=DateTime.MinValue;public DateTime Deadline=deadline,Created=now,Updated=now;public List<E> Events=[];public List<Child> Children=[]; } private sealed class Child(Guid id,string task,int attempt,string kind,Guid agentId,int agentRevision,Guid workflowId,int workflowRevision,string hash,Guid agentRunId,Guid commandId,JsonElement taskEnvelope,JsonElement dispatchArtifact){public Guid Id=id,AgentId=agentId,WorkflowId=workflowId,AgentRunId=agentRunId,CommandId=commandId;public string Task=task,Kind=kind,Hash=hash,Status="queued";public int Attempt=attempt,AgentRevision=agentRevision,WorkflowRevision=workflowRevision;public JsonElement TaskEnvelope=taskEnvelope.Clone(),DispatchArtifact=dispatchArtifact.Clone();} private sealed record E(long Sequence,string Type,string Hash,JsonElement Payload,DateTime At);
}
