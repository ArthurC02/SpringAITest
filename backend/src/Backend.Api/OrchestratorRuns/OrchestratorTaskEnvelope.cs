...
internal static class OrchestratorTaskEnvelope
{
    public static ValidatedOrchestratorTaskEnvelope Validate(JsonElement? raw)
    {
        ...
        var allowed = new HashSet<string>(new[] { "objective", "required_capabilities", "context", "context_provenance", "write_intent", "delegation_depth", "repair_of" }, StringComparer.Ordinal); var names = value.EnumerateObject().Select(x => x.Name).ToArray();
