...
public sealed class OrchestratorRunRepository(NpgsqlDataSource dataSource, IContextRepository? contexts = null, ContextEnrichmentState? contextState = null) : IOrchestratorRunRepository
{
    ...
    public async Task<OrchestratorChildResponse?> CreateChildAsync(string tenant, string user, Guid rootRunId, OrchestratorChildCreateRequest request, CancellationToken ct)
    {
        ...
        using var snapshot = JsonDocument.Parse(root.Snapshot); var value = snapshot.RootElement; var pin = OrchestratorRunSnapshotProjection.FindPin(value, kind, request.AgentId, request.AgentRevision); if (pin is null || request.TokenCap != pin.Value.TokenCap) { await tx.RollbackAsync(ct); return null; }
    ...
    public async Task<OrchestratorRunWriteResult> TransitionAsync(string tenant, string user, Guid rootRunId, OrchestratorRootTransitionRequest request, CancellationToken ct)
    {
        ...
        {
            return new(OrchestratorRunWriteStatus.InvalidState, Message: "Invalid root transition");
        }
        await using var c = await dataSource.OpenConnectionAsync(ct); await using var tx = await c.BeginTransactionAsync(ct);
    ...
    {
        if (request.ContextRound < 1 || request.CurrentContext is not { ValueKind: JsonValueKind.Object } || JsonUtf8.ByteCount(request.CurrentContext.Value) > 65_536) return null;
    ...
    private static async Task<ChildSource?> LoadChildSourceAsync(NpgsqlConnection c, NpgsqlTransaction tx, string tenant, Guid agentId, int revision, OrchestratorSnapshotPin pin, CancellationToken ct)
    {
