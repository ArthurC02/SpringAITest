...
public static class IdentityHeaders
{
    ...
    /// <summary>
    /// Workflow designer management is intentionally capability based: a tenant ADMIN is not
    /// implicitly a SYSTEM_ADMIN.  The Platform regenerates this header from repeated signed
    /// JWT claims; keep the backend check ordinal/exact and fail closed when the claim is absent.
    /// </summary>
    public static bool HasCapability(this HttpRequest request, string capability)
        => request.UserCapabilities().Contains(capability, StringComparer.Ordinal);
