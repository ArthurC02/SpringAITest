...
- `tests/Platform.Service.Tests` + `tests/Platform.Web.Tests` — xUnit, 279 tests total (Service 162 + Web 117), hand-written fakes (no mocking library).
...
- **Observability:** both blocking and streaming endpoints emit a business `Activity` span; .NET → OpenTelemetry → OTLP → Langfuse. Actual wiring happens in `Program.cs` reading env `LANGFUSE_OTEL_ENDPOINT` and `LANGFUSE_OTEL_AUTH` (no `OpenTelemetry` config section in appsettings.json — it's all env-driven). LiteLLM's `success_callback` covers token/cost as the second observability path. All 5xx errors return a fixed generic Chinese message to the client (details only in app logs), no exception details exposed.
