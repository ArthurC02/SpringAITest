...
- **Skill proxy routing:** `SkillController` routes CRUD (`POST`/`PUT`/`GET`/`DELETE /api/skills*`) and export (`GET /api/skills/{name}/export`, streams the backend zip through verbatim as `application/zip`) to backend; `NodeController` routes catalog (`GET /api/nodes`) and invocation (`POST /api/skills/{name}/invoke`, `/api/skills/validate`, `/api/skills/catalog`) to workflow. Both paths require `Authorization: Bearer` (JWT from user login). Workflow errors (unparseable YAML, missing nodes) surface as `400`/`422` to the client; if workflow is unreachable at invoke time → `502`.
