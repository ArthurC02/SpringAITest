...
public sealed class AgentFrameworkLlmAgent : ILlmAgent
{
    ...
    {
        var response = await _agent.RunAsync(ToChatMessages(messages), null, null, ct);
    ...
    {
        await foreach (var update in _agent.RunStreamingAsync(ToChatMessages(messages), null, null, ct))
        {
