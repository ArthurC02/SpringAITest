...
public interface IChatService
{
    ...
    /// </summary>
    Task<ChatResponse> ChatAsync(string message, string? userId, string? conversationId, UserContext? userCtx = null, CancellationToken ct = default, Guid? orchestratorId = null);

    /// <summary>串流式聊天:逐塊吐出回覆;串流完成時才持久化串接後的全文與更新記憶。userCtx 同上。</summary>
    IAsyncEnumerable<string> StreamChatAsync(string message, string? userId, string? conversationId, UserContext? userCtx = null, CancellationToken ct = default, Guid? orchestratorId = null);
