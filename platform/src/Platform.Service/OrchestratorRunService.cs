    ...
    ILogger<OrchestratorRunService> logger) : IOrchestratorRunService
{
    ...
    public async Task<AgentProxyResponse> StartAsync(Guid id,string? message,string? conversation,string? key,UserContext ctx,CancellationToken ct=default)
    {
    ...
    private async Task<AgentProxyResponse> Send(HttpMethod method,string path,UserContext ctx,object? body,string? key,CancellationToken ct)
    {
        ...
        if(!string.IsNullOrWhiteSpace(key))request.Headers.TryAddWithoutValidation("Idempotency-Key",key);
        var (status,responseBody,etag)=await backend.SendForProxyAsync(request,FailurePrefix+"Backend ",ct);
    ...
    private Task DispatchBestEffortAsync(Guid runId,Guid commandId,UserContext ctx,CancellationToken ct)
        => InternalRequest.KickBestEffortAsync(workflow,
            workflowOptions.BaseUrl.TrimEnd('/')+$"/orchestrator-runs/{runId:D}/dispatch",
            workflowOptions.InternalToken,ctx,
            new { command_id=commandId.ToString("D"), context=new { } },Json,logger,
            $"Root Workflow dispatch for run {runId:D}",ct);
}
