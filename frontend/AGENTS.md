...
- `src/components/` — `AuthPage`, `AppShell` (sidebar + four-view switching + CopilotKit readables/actions), `ChatView`, `DocumentsView`, `AnalysisView`, `ConfigView` (three tabs: `skill` → `SkillHome`, `nodeParams` → `NodeParamsTab`, `general` → `GeneralConfigTab` internal function); Skill-related: `AdvancedSkillEditor`, `SimpleSkillEditor`, `SkillRunPanel`, `SkillHistory`, `NodeCatalog`, `YamlEditor` (syntax highlighting), `TraceView` (invoke results).
