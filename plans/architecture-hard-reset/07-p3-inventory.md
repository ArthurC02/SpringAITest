...
- **revisions 歸屬**:`plans\skill-concept-realignment\02-spec.md:95`「revisions/restore 一律留在 Skill 面,BW 不提供 revisions 路由」 vs `plans\architecture-hard-reset\02-spec.md:88`「`/api/business-workflows*` … revisions」。後者較新(hard-reset 於 `01-plan.md:3` 明示 supersede),且 `02-spec.md:61-68` 已為 BW 建獨立 revision 表 —— 以 hard-reset 為準,但 skill-concept 那份未加 superseded 標記。**預決(2026-08-05):以 hard-reset 為準(已於 2026-08-09 整併時完成:superseded 標記落在 plans/skill-concept-realignment/01-plan.md 附錄)**,免得後續實作者誤讀舊規格。
