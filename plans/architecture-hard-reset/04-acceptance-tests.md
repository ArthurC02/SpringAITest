...
| P3-R2-01 | Evidence contract | The reviewed contract fixes bounded dimensions, authoritative counting points, deduplication rules, bundle schema, consumer attestation, observation integrity, rollback proof, and explicit approval states. Compile-time legacy inventory is not accepted as usage evidence. |
| P3-R2-02 | Runtime observation | A complete-version production window reports every public compatibility operation and internal alias/unified invoke separately; all Business Workflow write attempts under public `/api/skills*` are zero across every outcome, and every nonzero read/invoke has an owner. |
| P3-R2-03 | Consumer and rollback proof | Repository consumers are migrated or retained only when they do not depend on an operation being removed, every known external owner attests, no consumer remains `unknown`, and a named owner verifies the rollback build/procedure before the recorded rollback window ends. C8 remains blocked until that window has ended. |
| P3-R2-04 | C8 decision | A named human approval references the immutable evidence-bundle digest and authorizes only Business Workflow narrowing under public `/api/skills*`; alias and unified-invoke retirement remain unauthorized. |
