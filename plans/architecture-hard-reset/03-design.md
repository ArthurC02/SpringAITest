...
Extras detection scans only relations owned by no extension: `pg_class` entries in `public` with `relkind IN ('r','v','m','S')` whose `pg_depend` row has no `deptype = 'e'` link to an allowlisted extension. Objects that `pg_depend` attributes to an allowlisted extension — pgvector's types, operators, operator classes, casts, and functions, all of which install into `public` because `CREATE EXTENSION` is issued without a `SCHEMA` clause — are never evaluated against the table/view/sequence allowlist and never counted as unexpected. Without this rule a correctly provisioned database aborts the reset.

The same extension-ownership exclusion governs the type/function/operator categories of the schema fingerprint below: extension-owned catalog objects are represented only by the extension's name, version, and installation schema, never enumerated individually.
