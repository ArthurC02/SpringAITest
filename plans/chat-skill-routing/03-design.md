...
以 MCP 核對於撰寫當下成立;**後續 copilot-shared-core P4 已使下表漂移**——`BuildTools`/`TryRouteAndExecuteAsync`/`InvokeSkillToolAsync` 現位於 `SkillRoutingAgent.cs`,`WorkflowService.cs` 已更名 `WorkflowEngineClient.cs`,原生 function-calling 路徑已整段移除。落地時以下錨點為準:
