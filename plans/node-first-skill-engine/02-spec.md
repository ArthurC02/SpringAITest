...
| **Harness**           | 節點標準執行殼。所有節點一律經 Harness 包裝執行，提供 trace、逾時、I/O 契約驗證、fatal 短路與 Tool 注入。**歷史用語警示**:本文件『Harness』指每節點執行殼,對應現況的 **Node Shell**(`engine/node_shell.py`);現行『Harness』另指 D4 固定骨架(`runtime/graph.py`),見根 AGENTS.md。 |
...
```yaml
name: kb-query                 # ^[a-z0-9]([a-z0-9-]*[a-z0-9])?$(agentskills.io kebab-case,見 §9 回填),租戶內唯一
...
| D2  | §2.2 / 03-design §2                | 節點搬至 `app/nodes/kbquery/`                                   | 節點就地留在 `app/kbquery/nodes/`,僅補 `@node` 宣告                                                                                                               | 搬檔會改動既有測試的 import 路徑,違反 AT1-07;§2.2 明文「節點函式本體不改」,搬檔非硬性要求                                                                      | **合理補全**(建議回填 03-design 的路徑描述)<br>**二次更新(現況)**:此後又完成了原規格 §2.2 所要求的搬遷,節點與領域契約已全數移至 `workflow/app/nodes/kbquery/`,`app/kbquery/` 已刪除;D2 的『合理補全』裁決已被現況覆蓋。 |
...
| D8  | 01-plan §4(歷史,原亦見已封存刪除的 skill-authoring 計畫舊 §5) | 匯出 `SKILL.md` + `scripts/main.py`                             | 匯出 `SKILL.md` + `skill.yaml`；YAML 原文逐 byte 保存                                                                                                             | 含 node/tool/branch/loop 的流程無法忠實轉成 standalone Python；`skill.yaml` 才是可移植且可稽核的權威 definition                                                | **合理補全**(兩份計畫與驗收已同步)                                                                     |
