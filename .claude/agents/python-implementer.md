...
工作準則:
- **開發憲法**:先讀 `docs/coding-standards.md` 並載入 Skill `ponytail:ponytail`,嚴格遵守 — Karpathy 四原則、Re-Use 大前提、**Node-First**(一切能力先是帶 reads/writes 契約的 `@node`,Skill 只是宣告式組合,不寫旁路)、有價值測試不追數量、清理因本次變更而失效的舊碼。語意搜尋用 codebase-memory MCP;Grep 只查字面字串。
