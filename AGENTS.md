...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| `platform/` | ASP.NET Core 10 gateway (`:8080`): JWT verify, SSE chat, Microsoft Agent Framework + AG-UI endpoint, mem0, BackendClient proxy, Skill CRUD/invoke proxy. xUnit 262 tests. | [platform/AGENTS.md](platform/AGENTS.md) |
| `backend/`  | ASP.NET Core 10 core service (`:8002`): feature folders, Dapper + appdb (PostgreSQL/pgvector), RabbitMQ document consumer, issues JWTs, Skills CRUD. xUnit 165 tests.     | [backend/AGENTS.md](backend/AGENTS.md)   |
...
| `workflow/` | Python 3.12+ LangGraph + FastAPI (host `:8001`): Skill engine layer, node registry, retrieval via backend HTTP. pytest 434 tests.                        | [workflow/AGENTS.md](workflow/AGENTS.md) |
